...
async function restoreCacheV2(
  ...
  try {
    ...
    if (options?.lookupOnly) {
      core.info('Lookup only - skipping download')
      return cacheEntry.metadata?.key
    }
    ...
    for (const cachePath of cachePaths) {
      ...
      await s3Client.downloadFromS3(s3Config.bucketName, s3Key, cachePath)
    }
...
async function saveCacheV2(
  ...
  let success = 1
  for (const cachePath of cachePaths) {
    const s3Key = `${key}:${cachePath}` // '.gz' will be appended in uploadToS3 if compressed
    try {
